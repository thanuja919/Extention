...
public class DefaultPasswordHistoryDataStore implements PasswordHistoryDataStore {
    ...
    private int minPasswordChangeHours;

    public DefaultPasswordHistoryDataStore(String digestFunction, int maxHistoryCount, int minPasswordChangeHours) {
        this.digestFunction = digestFunction;
        this.maxHistoryCount = maxHistoryCount;
        this.minPasswordChangeHours = minPasswordChangeHours;
    }
    ...
    public void store(User user, Object credential) throws
        ...
        try {
            ...
            //Password cannot have been previously used in the past minPasswordChangeHours (24) hours
            Timestamp timeADayBefore = new Timestamp(new java.util.Date().getTime() - minPasswordChangeHours * 3600 * 1000l);
            while (resultSet.next()) {
                ...
                if (recordsToDelete.size() > 0) {
                    ...
                    if (storedTimeStamp.after(timeADayBefore)) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                if (storedHistoryCount <= maxHistoryCount) {
                    ...
                    Timestamp storedTimeStamp = resultSet.getTimestamp("TIME_CREATED");
                    if (storedTimeStamp.after(timeADayBefore) && isHistoryExists(resultSet.getString("SALT_VALUE"), resultSet.getString("HASH"), credential)) {
                        return false;
                    }
                }
            }
